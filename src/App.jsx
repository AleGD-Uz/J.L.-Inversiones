...
export default function App() {
    ...
    return (
        <div className="flex h-[100dvh] font-sans text-slate-800 overflow-hidden text-xs md:text-sm lg:text-base relative" style={{ backgroundImage: 'radial-gradient(circle at 15% 20%, rgba(20, 184, 166, 0.08) 0%, transparent 45%), radial-gradient(circle at 95% 15%, rgba(236, 72, 153, 0.12) 0%, transparent 50%), radial-gradient(circle at 80% 85%, rgba(139, 92, 246, 0.1) 0%, transparent 45%), radial-gradient(circle at 25% 80%, rgba(59, 130, 246, 0.06) 0%, transparent 40%), linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)' }}>
            ...
            <main className={`flex-1 overflow-y-auto relative p-4 md:p-6 bg-transparent ${activeTab === 'pos' ? 'lg:py-4 lg:px-2' : 'lg:p-8'}`}>
                ...
                {activeTab === 'pos' && (
                    <div className="w-full space-y-4 fade-in">
                        <header className="flex flex-col gap-2 mt-10 md:mt-0">
                            <h2 className="text-2xl md:text-3xl font-black text-slate-800">{editingOrderId ? `Editando Venta` : 'Punto de Venta'}</h2>
                            <AdvancedToolbar searchQuery={searchQuery} setSearchQuery={setSearchQuery} sortConfig={sortConfig} setSortConfig={setSortConfig} sortOptions={[{ value: 'name', label: 'Nombre' }, { value: 'price', label: 'Precio' }, { value: 'category', label: 'Categoría' }]} placeholder="Buscar producto..." />
                        </header>
                        <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar ">
                            {['Todos', ...new Set(products.map(p => p.category))].map(cat => (
                                <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-xl whitespace-nowrap text-sm font-bold ${selectedCategory === cat ? 'bg-teal-500 text-slate-900' : 'bg-white text-slate-600 shadow-sm'}`}>{cat}</button>
                            ))}
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 items-start pb-24 md:pb-0">
