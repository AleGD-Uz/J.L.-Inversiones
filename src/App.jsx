...
export default function App() {
    ...
    const handleSaveToPending = async () => {
        ...
        logActivity('Comanda', `Orden ${isUpdate ? 'actualizada' : 'enviada a cocina'}: ${desc}`);
    ...
    const handleSaveProduct = (e) => {
        ...
        const d = { id: editingProduct?.id || generateSecureId(), name: fd.get('name'), price: parseFloat(fd.get('price')), category: fd.get('category'), image: fd.get('image'), recipe: editingProduct?.recipe || [] };
        ...
        <div className="flex h-[100dvh] bg-slate-100 font-sans text-slate-800 overflow-hidden text-xs md:text-sm lg:text-base">
            <aside className={`fixed z-40 inset-y-0 left-0 w-64 bg-slate-900 text-white transform transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} shadow-2xl flex flex-col`}>
                <div className="p-6 flex items-center gap-3 border-b border-slate-700/50">
                    ...
                    <div>
                        <h1 className="text-xl font-bold tracking-tight">Yuya's</h1>
                        <FixedClock />
                    </div>
                ...
                <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">{[
                    ...
                    { id: 'bitacora', label: 'Bitácora', icon: <FileSearch size={20} /> }, // NUEVO TAB DE BITÁCORA
